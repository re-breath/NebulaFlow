...
def load_ky_running_average(kappa_file: Path, dt_ns: float) -> Tuple[np.ndarray, np.ndarray]:
    """读取 kappa 文件，返回 t(ns) 与 ky 的 running average 曲线。"""
    ...
    ky_ra = kappa['kyi_ra']+kappa['kyo_ra']
    return t, ky_ra
...
def plot_hnemd_ky_with_tail10_mean(
    out_png: str = "hnemd_mul_y.png",
    ...
    for f in run_files:
        ...
        t, ky_ra = load_ky_running_average(f, DT_NS)
        if t_ref is None:
            t_ref = t

        ky_curves.append(ky_ra)

        tail_mean = last_fraction_mean(ky_ra, tail_frac)
        run_tail_means.append((f.stem, tail_mean))
        print(f"{f.name} | <ky>_last{int(tail_frac*100)}% = {tail_mean:.6g}")

        ax.plot(t, ky_ra, color=COLOR_OTHERS, lw=1.4, alpha=0.35)

    if not ky_curves:
        raise RuntimeError("没有成功读取任何 kappa_i.out 文件，无法作图。")


    ky_mean = stack_mean_curve(ky_curves)

    min_len = ky_mean.size
    t_plot = (t_ref[:min_len] if t_ref is not None else np.arange(1, min_len + 1) * DT_NS)

    ax.plot(t_plot, ky_mean, color=COLOR_MEAN, lw=2.8, alpha=0.95, label="Mean of others")


    mean_tail_mean = last_fraction_mean(ky_mean, tail_frac)
    print(f"[平均曲线]  | <ky>_last{int(tail_frac*100)}% = {mean_tail_mean:.6g}")


    lines = [rf"$\langle \kappa_y \rangle_{{last\,{int(tail_frac*100)}\%}}$ = {mean_tail_mean:.4g}"]

    arr = np.array(run_tail_means, dtype=object)  # 每行 [name, value]
    np.savetxt("Everyhnemd.txt", arr, fmt="%s,%.6f", header="run,mean_ky_last10%", comments="")
...
if __name__ == "__main__":
    plot_hnemd_ky_with_tail10_mean(
        out_png="hnemd_mul_y.png",
