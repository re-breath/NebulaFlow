...
def load_kx_running_average(kappa_file: Path, dt_ns: float) -> Tuple[np.ndarray, np.ndarray]:
    """读取 kappa 文件，返回 t(ns) 与 kx 的 running average 曲线。"""
    ...
    kx_ra = kappa['kxi_ra']+kappa['kxo_ra']
    return t, kx_ra
...
def plot_hnemd_kx_with_tail10_mean(
    out_png: str = "hnemd_mul_x.png",
    ...
    for f in run_files:
        ...
        t, kx_ra = load_kx_running_average(f, DT_NS)
        if t_ref is None:
            t_ref = t

        kx_curves.append(kx_ra)

        tail_mean = last_fraction_mean(kx_ra, tail_frac)
        run_tail_means.append((f.stem, tail_mean))
        print(f"{f.name} | <kx>_last{int(tail_frac*100)}% = {tail_mean:.6g}")

        ax.plot(t, kx_ra, color=COLOR_OTHERS, lw=1.4, alpha=0.35)

    if not kx_curves:
        raise RuntimeError("没有成功读取任何 kappa_i.out 文件，无法作图。")


    kx_mean = stack_mean_curve(kx_curves)

    min_len = kx_mean.size
    t_plot = (t_ref[:min_len] if t_ref is not None else np.arange(1, min_len + 1) * DT_NS)

    ax.plot(t_plot, kx_mean, color=COLOR_MEAN, lw=2.8, alpha=0.95, label="Mean of others")


    mean_tail_mean = last_fraction_mean(kx_mean, tail_frac)
    print(f"[平均曲线]  | <kx>_last{int(tail_frac*100)}% = {mean_tail_mean:.6g}")


    lines = [rf"$\langle \kappa_x \rangle_{{last\,{int(tail_frac*100)}\%}}$ = {mean_tail_mean:.4g}"]

    arr = np.array(run_tail_means, dtype=object)  # 每行 [name, value]
    np.savetxt("Everyhnemd.txt", arr, fmt="%s,%.6f", header="run,mean_kx_last10%", comments="")
...
if __name__ == "__main__":
    plot_hnemd_kx_with_tail10_mean(
        out_png="hnemd_mul_x.png",
